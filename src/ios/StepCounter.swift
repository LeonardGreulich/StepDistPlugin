...
class StepCounter {
    ...
    func startStepCounting() {
        ...
        motionManager.startDeviceMotionUpdates(to: OperationQueue.current!) { (data, error) in
            if let motionData = data {
                self.processMotionDataGrav(x: motionData.gravity.x, y: motionData.gravity.y, z: motionData.gravity.z)
            }
        }
    }
    ...
    func getStepsAfter(_ date: Date) -> Int {
        ...
        guard allStepDatesAfter.count >= 1 else {
            return 0
        }
        
        var smoothingCompensation: Int = 0
        let rTInSeconds: Double = updateInterval*Double(rT)
        let secondsBetweenLastStepAndNow: Double = Date().timeIntervalSince(allStepDatesAfter.max()!)
        if secondsBetweenLastStepAndNow >= rTInSeconds*1 && secondsBetweenLastStepAndNow <= rTInSeconds*2 {
            smoothingCompensation = 2
        }
        
        return allStepDatesAfter.count+smoothingCompensation
    }
